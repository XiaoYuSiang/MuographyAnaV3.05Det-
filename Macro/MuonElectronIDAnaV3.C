...
const int ProcessNum = cpuCores;
const bool processShow   = false;
const int NumIDCase = 11, NumNoiCase = 6, NumLepCase = 5;
TFile *rotfile_0;
TCanvas *cpwvcase;
...
void IDStatistic(int iProc){
  int evChipNum = evOfIDFrame/cpuCores;
  int evRangeMin = iProc*evChipNum;
  int evRangeMax = iProc==cpuCores-1 ? evOfIDFrame :(iProc+1)*evChipNum;
  
  int   nH;
  bool  ElectronID, MuonID, NoiseID;
  short IDtight;
  Int_t *pwidth_,*pwidthS_;
  /*Adjust this funtion*/
  // cout<<"Drawing the pwidth of case: "<<IDCaseWord[iProc]<<endl;
  // ths[iCore]->Lock();
  TreeReader data(rotfile_0->GetName());
  for(Long64_t iev=evRangeMin;iev<evRangeMax;iev++){
    if(iev%1000 == 0) cout<<Form("\r%.5f%%",((iev-evRangeMin)*100.)/(1.*evChipNum))<<flush;
    data.GetEntry(iev);
    nH          = data.GetInt("nH");
    ElectronID = data.GetBool("ElectronID");
    MuonID     = data.GetBool("MuonID");
    NoiseID    = data.GetBool("NoiseID");
    IDtight    = data.GetShort("IDtight");
    pwidth_    = data.GetPtrInt("pwidth");
    pwidthS_   = data.GetPtrInt("pwidthScaleFactor");
    // for(int iHit=0; iHit<nH; iHit++)
      // hPwidthCases[0]->Fill(float(pwidth_[i])/float(pwidthScaleFactor_[i]));
    for(int i=0;i<nH;i++) hPwidthCases[0]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
    if(NoiseID){
      for(int i=0;i<nH;i++) hPwidthCases[1]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
      if(nH==1){
        for(int i=0;i<nH;i++) hPwidthCases[2]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
      }else if(nH==2){
        for(int i=0;i<nH;i++) hPwidthCases[3]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
      }else if(nH==3){
        for(int i=0;i<nH;i++) hPwidthCases[4]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
      }else{
        for(int i=0;i<nH;i++) hPwidthCases[5]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
      }
    }else if(MuonID){
      if(IDtight==5){
        for(int i=0;i<nH;i++) hPwidthCases[6]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
      }else if(IDtight==4){
        for(int i=0;i<nH;i++) hPwidthCases[7]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
      }else if(IDtight==3){
        for(int i=0;i<nH;i++) hPwidthCases[8]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
      }else if(IDtight==2){
        for(int i=0;i<nH;i++) hPwidthCases[9]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
      }
    }else if(ElectronID){
      for(int i=0;i<nH;i++) hPwidthCases[10]->Fill((1.*pwidth_[i])/(1.*pwidthS_[i]));
    }
  }
}
...
void MuonEleIDAna( const char *RFNStr,const char *TestMarker, const double eventgap) {
  ...
  rotfile_0 = TFile::Open(EventFuileName.data());
  ...
  cpwvcase = new TCanvas("cpwvcase","",1920,1080*2);
  cpwvcase->Divide(1,2);
  cpwvcase->cd(2);
  ...
  // fill histrogram
  TreeReader data(rotfile_0->GetName());
  evOfIDFrame = data.GetEntriesFast();
  cout<<"File: "<<rotfile_0->GetName()<<" is open\n    the evs is: "<<evOfIDFrame<<endl;
  ContralThread();
  // IDStatistic();
  for(int i=0;i<NumIDCase;i++){
    hPwidthCases[i]->Draw(Form("hist%s",i==0?"":"same"));
  }
