...
void VerticalTracksSub(const char *RFNStr,const bool testMode, const int ChipIndex=0){
  ...
  for (Long64_t ev = evRangeMin; ev <evRangeMax; ++ev) {//evsVT; ++ev) {
    ...
    for(int i2=0 ; i2<VTD.eventHit ; i2++){
      ...
      // cout<<105<<"  "<<VTD.board_[i2]<<"\t"<<BDCheck(VTD.board_[i2])<<"\t"<<GID<<"\t"<<LID<<endl;
      ...
      if( BLayCheck(VTD.BLay, VTD.iHit ) ){//return BLay and iHit
        BLayCheck(VTD.BLay, VTD.iHit );
        VTD.TrackIndex ++;
        tree->Fill();
      }
      // cout<<ev<<"\t"<<VTD.board[0]<<"\t"<<VTD.BLay<<"\t"<<VTD.iHit<<endl;
      BoolnL =0;
      VTD.Clear();
...
void VerticalTracksSubHAdd(const char *RFNStrIn, const string VTAFile){
  const string RFNStr = Form(RFNStrIn);
  ...
  cout<<"cpuCores: "<<cpuCores<<endl;
  for(int iCore = 0; iCore<cpuCores; iCore++){
    string TrackDataSub   
      = Form("%sTracksGT%s_t%02.f.root",DirOperate,RFNStr.data(),iCore*1.);
    cout<<"Get file: ic-"<<iCore<<" = "<<TrackDataSub<<" to HADD\n";
...
void VerticalTracksSubControl(const char *RFNStr,const bool testMode){
  ...
  string TrackData = Form("%sTracksGT%s.root",DirOperate,RFNStr);
  Long64_t size = FileSize(TrackData.data());
  if(size > 0){
    TFile *rottmp = new TFile(TrackData.data(),"open");
    TObjString *FlagFinish;
    bool usefulFile = (FlagFinish =(TObjString*) rottmp->Get("FlagFinish"));
    if(usefulFile){
      cout<<"The "<<TrackData.data()
...
void VerticalTracksSubControlSepR(const char *RFNStr,const bool testMode){
  for(int irun=0; irun<RunNums; irun++){
    int tmpRID = RunID[irun];
    string GapTData = Form("%sGapT%s%d.root",DirOperate,RFNStr,tmpRID);
    Long64_t size = FileSize(GapTData.data());
    if( size <= 0){
      cout<<"The GapT file is not exist in this run, skip run proc. of the VertiTrackAna"<<endl;
      continue;
    }
    string TrackData = Form("%sTracksGT%s%d.root",DirOperate,RFNStr,tmpRID);
    size = FileSize(TrackData.data());
    if(size > 9000){
      TFile *rottmp = new TFile(TrackData.data(),"open");
      TObjString *FlagFinish;
      bool usefulFile = (FlagFinish =(TObjString*) rottmp->Get("FlagFinish"));
    ...
    string VTAFile = Form("%sTracksGT%s%d.root",DirOperate,RFNStr,tmpRID);
    cout<<"Merge the files to be: "<<VTAFile.data()<<endl;
    
    VerticalTracksSubHAdd(Form("%s%d",RFNStr,tmpRID),VTAFile);
    cout<<"Finish the files marge and rm the tmp datas\n";
  }
}