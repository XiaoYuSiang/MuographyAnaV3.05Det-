...
void TrackAnaSubV3(const string InputGapTFile, const string Tmp_ETrFile_hash, const string evhash, const string OsName, const int ir, const int ev_Fir, const int ev_Fin,const double trigger) {
  // cout<<"44- ir = "<<ir<<endl;
  // return;
  char version[8] = {};
  ...
  /* cout<<TnHR[ir]<<"\t"<<TnH0[ir][0]<<"\t"<<TnH0[ir][1]<<"\t"<<TnH0[ir][2]<<"\t"<<TnH0[ir][3]<<"\n"; */
  for (Long64_t ev = ev_Fir; ev <ev_Fin; ++ev) {
  // for (Long64_t ev = ev_Fir; ev <1008034; ++ev) {
    data.GetEntry(ev);
    unixtime_ = data.GetLong64("unixtime");
    if(ev%1000 == 0) 
      cout<<Form("\r%.5f%%",((ev-ev_Fir)*100.)/(1.*(ev_Fin-ev_Fir)))
          <<Form("   %d",unixtime_)
          <<flush;
    O0++;
    ...
    if( nH > TnHR[ir] ) continue;
    O2++;
    
    nHk[0]       = data.GetInt("nH0");
    nHk[1]       = data.GetInt("nH1");
    
    if( nHk[0] > TnH0[ir][0] ) continue;
    if( nHk[1] > TnH0[ir][1] ) continue;
    O3++;
    
    nHk[2]       = data.GetInt("nH2");
    nHk[3]       = data.GetInt("nH3");

    if( nHk[2] > TnH0[ir][2] ) continue;
    if( nHk[3] > TnH0[ir][3] ) continue;
    O4++;
    ...
    for(int iHit=0 ; iHit<nH ; iHit++){
      ...
      // cout<<board_[iHit]<<"\t"<<channel_[iHit]<<"\t"<<GID<<"\t"<<MScints[GID].iX<<"\t"<<MScints[GID].iY<<"\t"<<MScints[GID].iZ<<endl;
    }
    ...
    if(int(vTrack3D.size())<1) continue;
    float BestErr = 1.E+8;
    int BestLineIndex = -1; 
  ...
  /* cout<<O0<<"\t"<<O1<<"\t"<<O2<<"\t"<<O3<<"\t"<<O4<<"\t"<<O5<<"\t"<<TrackIndex; */
  outOs.close();
  // rotfile->Write();
  rotfile->WriteObject(tree,"t");
  rotfile->Close();
  // tree->Delete();
  // FATAL(Form("\nFinish the create of : %s",rotfile->GetName()));
}
...
void TrackAnaSubHAdd(const string TargetETrFile, const vector<string> vChipETrFiles, vector<int> StaTrackIndex) {
  ...
  cout<<"Going the 422 line for Hadd"<<endl;
  cout<<"Going the 422 line for Hadd"<<endl;
  cout<<"Going the 422 line for Hadd"<<endl;
  //Data Variable
  //new Tree unit and tree unit
