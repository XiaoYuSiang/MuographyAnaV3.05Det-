...
void EventAna(){ LastVerShow(); }
void EventAnaV2(){ LastVerShow(); }
void EventAnaV3(const int indexi=28, const int indexf=29, const int unixtimeini = unixtimei, const int unixtimefin = unixtimef, const char * runName="-NA-" ) { LastVerShow(); }
void EventAnaV4(
  const int iCore, const int iProc, const int index = 28, 
  const char * runName="-NA-") { LastVerShow(); }
...
{//140
  ...
  cout<<"Checking the option of file: "<<rtfN.data()<<"\tis"<<size<<"Byte"<<endl;
  
  if(RecreateOption!='R'&&RecreateOption!='r'){
    // cout<<"\nL91!!!!!!!!!!!!!!!!!!!!!!!!!! iCPU = "<<iProc<<endl;
    if(size > 700){
      ths[iProc]->Lock();
      TFile *rottmp = new TFile(rtfN.data(),"open");
      // if(!rottmp->Get("FinishOrNot")) 
...
void *process(void *arg){
  ...
  /*Adjust this funtion*/
  EventAnaV5(iCore, iProc, gapIndexNow, Form("Run%d",iProc));//, const char * runName="-NA-" ) {//140
