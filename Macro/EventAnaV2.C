...
void EventAnaV2(const int indexi=28, const int indexf=29, const int unixtimeini = unixtimei, const int unixtimefin = unixtimef, const char * runName="-NA-" ) {//140
  defstyle(); 
  double eventGap[50]={};
    ...
    for (Long64_t ev = 0; ev < evs ; ++ev) {
      ...
      for( int i=0 ; i<nHits; i++){
       // if (fabs(dtime0-dtime_[i])>eventGap[i0]){
        ...
        if((nHit!=0&&dtcnt[nHit]>eventGap[i0])||(MCMode&&frameLast!=frame_)){
          ...
          if ( TriggerS < nHit&&nHit < TriggerL){
