...
void CaConvertor(){
  FATAL("Can't direct to use this script, please use:\n  CaConvertorV2.C+(TS.filename,TS.ofile,I.FrameNumEsti)");
}
void CaConvertorV2(
  TString filename="/data4/YuSiang/DaXi/20220907_Run1_0_Mu.txt", 
  TString ofile="/data4/YuSiang/DaXi/Ana2/rootfile/20220907_Run1_0_Mu_Test.root",
  int FrameNumEsti = 10000
) {

  ifstream infile;
  infile.open(filename);
  string outExceptionFileName = Form("%sCaConvertProblem.dat", DirRotfile);
  ...
  OutStrs+="BadLine: [linebad] : [tc>2.56E9] : [tcM>2E9&tc<1E9] : [UT-pc!=pcOff]\n";
...
void MuoCaConvertor(const char OPT=' '){
  FATAL("Please use the last version: MuoCaConvertorV2");
}
void MuoCaConvertorV2(const char OPT=' '){
