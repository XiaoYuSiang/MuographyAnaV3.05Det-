...
int CalFileLine(const char* infilename, int *casePos, char OPT = 'C'){
  ...
  for(int iCasePos = 0; iCasePos<10; iCasePos++)
    casePos[iCasePos] = -1;
...
void SaveRunData(const char *infileName, CRunData &runData){
  runData.sortRunInf();
...
void ReadRunData(const char *infileName, CRunData &runData, char OPT='-'){
  ...
  // for(int i = 0; i<10;i++)cout<<casePos[i]<<"\t";
...
void RunFileNumCheck( int &iRunStart, int &iRunFinal ){
  for( iRunStart = 0 ; true ; iRunStart++){
    char tmpfile[250];
    sprintf(tmpfile,"%sSetup_Run%d.txt",DirRunData,iRunStart);
  ...
  for( iRunFinal = iRunStart ; true ; iRunFinal++){
    char tmpfile[250];
    sprintf(tmpfile,"%sSetup_Run%d.txt",DirRunData,iRunFinal);
...
int TillFileCheck(const char *dataPath, const int iRunStart, const int iRunFinal){
  ...
  for(; lastTillFileIndex>iRunStart;lastTillFileIndex--){
    ...
    for(int icase = 0; icase<2; icase++){
      char tmpfile[250];
      sprintf(tmpfile,"%sSetup_Till_Run%d.%s",dataPath,lastTillFileIndex,Filetype[icase]);
      int  casePos[10]={};
    ...
    char tmpfile[250];
    sprintf(tmpfile,"%sSetup_Till_Run%d.root",dataPath,lastTillFileIndex);
    cout<<"Checking: "<<tmpfile<<endl;
    Long64_t SizeOfFile = FileSize(tmpfile,'Q');
    if(SizeOfFile>1000){
...
void RunDataSortAna(){
  ...
  if(lastTillFileIndex!=iRunFinal){
    if(lastTillFileIndex!=0){
      sprintf(infileName,"%sSetup_Till_Run%d.txt",DirRunTillfile,lastTillFileIndex);
      ReadRunData(infileName, runData,'q');
    }
    for(int i=lastTillFileIndex; i<=iRunFinal;i++){
      
      sprintf(infileName,"%sSetup_Run%d.txt",DirRunData,i);
      sprintf(outfileName,"%sSetup_Till_Run%d",DirRunTillfile,i);
      ...
      SaveRunData(outfileName, runData);
    }
  }
