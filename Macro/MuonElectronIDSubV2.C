...
void MuonElectronIDSub(){
  FATAL("Error: Please use the last version MuonElectronIDSubV2");
}

void MuonElectronIDSubV2(const char *RFNStr,const char *TestMarker, const int ChipIndex=0){
  ...
  for (Long64_t ev = evRangeMin; ev <evRangeMax; ++ev) {//evsID; ++ev) {
    ...
    if(ev%1000 == 0) cout<<Form("\r%.5f%%",((ev-evRangeMin)*100.)/(1.*evChipNum))<<flush;
    int nH = MEIDData.nH;
    MEIdentify( MEIDData );
...
void MuEleIDSubHAdd_easy(const char *RFNStr,const char *TestMarker, const string MEIDFile){
  ...
  string MEIDDataFin = Form("%s%sEventsGT%s.root",DirOperate,TestMarker,RFNStr);
  ...
  TFile *rotfile = TFile::Open(MEIDDataFin.data(),"Update");
  TObjString *FlagFinish = new TObjString("true");
  rotfile->WriteObject(FlagFinish,"FlagFinish");
  rotfile->Close();
}
