...
void FitTrackResAna(
  ...
  FATAL("Please use the last Ver. : FitTrackResAnaV2(c.i. indexGap, c.d. binWidth, c.c opt, c.b Rotate)");
}
...
//561000*2
  ...
  for(int iF=0;iF<1;iF++){
    ...
    TH2F *TDDD = SetTDDDModel("TDDD","#font[12]{Tracking result direction distribution}",binWidth);
    TH2F *TDDDNorEff = SetTDDDModel("TDDDNorEff","#font[12]{Normalize by Efficiency}",binWidth);
    TH2F *TDDDNorEffGeo = SetTDDDModel("TDDDNorEffGeo","#font[12]{Normalize by Eff.& Geometry Factor}",binWidth);
    TH2F *TDDDNorEffGeoThe = SetTDDDModel(Form("TDDDNorEffGeoThe"),"#font[12]{Normalize by Eff.& G.Fac & Angular Factor}",binWidth);
    TH2F *TDDDNorEffGeoTheTim = SetTDDDModel(Form("TDDDNorEffGeoTheTim"),"#font[12]{Normalize by Eff.& G.Fac & A.Fac& WorkTime}",binWidth);
    ...
    TFile *ScaleFacFile = new TFile(Form("%sOAngComFacBXY%03.f.root",DirIMFdat,NumBins*1.),"READ");
    ...
    TDDDNorEffGeoTheTim->GetZaxis()->SetRangeUser(0.0000,0.01000);
    ...
    TH2F *TDDDP = SetTDDDModel("TDDDP","#font[12]{Tracking result direction distribution}",binWidth);
    TH2F *TDDDPNorEff = SetTDDDModel("TDDDPNorEff","#font[12]{Normalize by Efficiency}",binWidth);
    TH2F *TDDDPNorEffGeo = SetTDDDModel("TDDDPNorEffGeo","#font[12]{Normalize by Eff.& Geometry Factor}",binWidth);
    TH2F *TDDDPNorEffGeoThe = SetTDDDModel(Form("TDDDPNorEffGeoThe"),"#font[12]{Normalize by Eff.& G.Fac & Angular Factor}",binWidth);
    TH2F *TDDDPNorEffGeoTheTim = SetTDDDModel(Form("TDDDPNorEffGeoTheTim"),"#font[12]{Normalize by Eff.& G.Fac & A.Fac& WorkTime}",binWidth);
    ...
    DrawPdfPng(C,Form("%sSADxDzVSDyDzP%s",DirRes_FluxCal,GraphEndStamp));
    ...
    DrawPdfPng(C,Form("%sSBDxDzVSDyDzPNorEff%s",DirRes_FluxCal,GraphEndStamp));
    ...
    DrawPdfPng(C,Form("%sSCDxDzVSDyDzPNorEffGeo%s",DirRes_FluxCal,GraphEndStamp));
    ...
    DrawPdfPng(C,Form("%sSDDxDzVSDyDzPNorEffGeoThe%c%s",DirRes_FluxCal,opt,GraphEndStamp));
    ...
    DrawPdfPng(C,Form("%sSEDxDzVSDyDzPNorEffGeoThe%cTim%s",DirRes_FluxCal,opt,GraphEndStamp));
    
    
    vector<float>vRxyBinMrad;
    for(int i=0; i < int(vRxyBin.size());i++)
      vRxyBinMrad.push_back(atan(vRxyBin[i])*1000);
    ...
    h2->GetZaxis()->SetTitle(Form("Number/Eff./GeoAreaFactor/%s/Worktime",AngTerm));
    ...
    h2->Draw("colz");
    DrawPdfPng(C,Form("%sSEDxDzVSDyDzPNorEffGeoThe%cTim%sMRAD",DirRes_FluxCal,opt,GraphEndStamp));
...
void  FitTrackResAnaAll( const int indexGap=28, const bool Rotate=false){
  ...
  for(int i=0; i<FluxBinDXYDZCases;i++){
    ...
    string cmdlineB = 
      string(Form(" &&root -l -b %sFitTrackResAnaV2.C+\\\(%d,%f,\\\'",DirMacros,indexGap,FluxBinDXYDZWidth[i]));
