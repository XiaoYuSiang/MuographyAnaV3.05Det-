...
//561000*2
  ...
  for(int iF=0;iF<1;iF++){
    ...
    if(Rotate) t->Draw("atan(DxDzPos):-atan(DyDzPos)>>TDDDP",Form("(CodeEmp==0)&&%s",TriLesDxDzADyDzPos),"colz");
    else t->Draw("atan(DyDzPos):atan(DxDzPos)>>TDDDP",Form("(CodeEmp==0)&&%s",TriLesDxDzADyDzPos),"colz");
    ...
    if(Rotate) t->Draw("atan(DxDzPos):-atan(DyDzPos)>>TDDDPNorEff",Form("(CodeEmp==0&&%s)*EffWei",TriLesDxDzADyDzPos),"colz");
    else t->Draw("atan(DyDzPos):atan(DxDzPos)>>TDDDPNorEff",Form("(CodeEmp==0&&%s)*EffWei",TriLesDxDzADyDzPos),"colz");
    for(int ixbin=1; ixbin<=binNFX; ixbin++){
      for(int iybin=1; iybin<=binNFY; iybin++){
