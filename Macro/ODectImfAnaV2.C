...
void ODectImfAnaV2(char Opt='q') {
  ...
  string ChDataPath_tmp = Form("%stmp_PositionDataPath.dat",DirOperate);
  system(Form("ls ./DetInforSet/*%s*.dat>%s",detVer,ChDataPath_tmp.data()));
  ifstream in(ChDataPath_tmp.data());
  string PosFilePath; in>>PosFilePath; //in.close();
  system(Form("rm %s",ChDataPath_tmp.data()));
  if(FileSize(PosFilePath.data())==0) 
    FATAL(Form("Error: Not find the Position imformation data *%s*.dat in the ./DetInforSet/",detVer));
  else 
    cout<<"InFo: Find out the detector by: "<<PosFilePath<<endl;
  
  
  string Path_rotfile = Form("%sSciPosDat%s",Path_ODetIMFData,version);
  TFile *rotfile = new TFile(Form("%s.root",Path_rotfile.data()),"RECREATE");
  ofstream outfiletxt(Form("%s.txt",Path_rotfile.data()));
  ofstream outfilecsv(Form("%s.csv",Path_rotfile.data()));
  ...
  ifstream in2(PosFilePath.data());
  string tmp_str = "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee";
  ...
  vector<scintillator> vMTBPos;
  float VBxm[NumBD], VBym[NumBD], VBzm[NumBD];
  int   Vface[NumBD], VBID[NumBD];

  // while(getline(in2,tmp_str))
    // cout<<tmp_str<<endl;
  
  
  while( getline(in2,tmp_str)) {
    stringstream iss(tmp_str);
    int face;
    /*if(!*/
    iss>>Scints.GeneralID>>Scints.boardID>>Scints.channelID>>Scints.BiX>>Scints.BiY>>Scints.BiZ>>Scints.iX>>Scints.iY>>Scints.iZ>>Scints.pX>>Scints.pY>>Scints.pZ>>face;//) break;
    // cout<<"Origin: "<<tmp_str<<endl;
    if(face==1) Scints.face = true;
    else Scints.face = false;
    // cout<<Scints.GeneralID<<"	"<<Scints.boardID<<"	"<<Scints.channelID<<"	"
      // <<Scints.BiX<<"	"<<Scints.BiY<<"	"<<Scints.BiZ<<"	"<<Scints.iX<<"	"
      // <<Scints.iY<<"	"<<Scints.iZ<<"	"<<Scints.pX<<"	"<<Scints.pY<<"	"
      // <<Scints.pZ<<"	"<<Scints.face<<"	"<<endl;
    Scints.SetName(Form("Scintillator-%03.f",Scints.GeneralID*1.0));
    Scints.PwWei = 1;
    tPos->Fill();
    Scints.Sci_TxtPrint(outfiletxt);
    Scints.Sci_CsvPrint(outfilecsv);
    if(Scints.channelID==0||Scints.channelID==15){
      VBxm[BDCheck(Scints.boardID)] += Scints.pX;
      VBym[BDCheck(Scints.boardID)] += Scints.pY;
      VBzm[BDCheck(Scints.boardID)] += Scints.pZ;
      Vface[BDCheck(Scints.boardID)] = Scints.face;
    } 
  }
  
  TemSen.SetBIndex(Scints.BiX,Scints.BiY,Scints.BiZ);
  for(int ibd=0;ibd<NumBD;ibd++){
    
    for(int itch=0; itch<=5; itch++){
      TemSen.SetID_Face_Ver(ibd*6+itch,BD[ibd],itch,Vface[ibd],version);

      TemSen.MoveBPosition(VBxm[ibd]/2.,VBym[ibd]/2.,VBzm[ibd]/2.);
      if(Opt!='q') Scints.Sci_CoutPrint();
      tPot->Fill();
    }
  }
  cout<<Form("%s is RECREATED.",Path_rotfile.data())<<endl;
  
  rotfile->Write();
  cout<<"Finish the ODectImfAnaV2()"<<endl;
}
...
void TillRunODInf(){
  ...
  for(int iR = iRunStart ; iR < iRunFinal ; iR++){
    ...
    */
    ...
    for(int i = 0; i<tPos->GetEntries(); i++){
      ...
      tRun->GetEntry(i);
      tRPos->Fill();
      // cout<<Scintstmp.GeneralID<<"	"<<Scintstmp.boardID<<"	"<<Scintstmp.channelID<<"	"<<Scintstmp.iX<<"	"<<Scintstmp.iY<<"	"<<Scintstmp.iZ<<endl;
      // cout<<RunScints.GeneralID<<"	"<<RunScints.boardID<<"	"<<RunScints.channelID<<endl;
    }
    rotfile->Write();
  ...
  cout<<"Finish the TillRunODInf()"<<endl;
}
