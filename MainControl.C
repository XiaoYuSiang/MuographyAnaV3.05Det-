...
void MainControl(
  char *path_Lib    , char *path_Raw, char *path_Hk,
  ...
  const string runOPT = "nLrP")
{
  ...
  ofstream outSetting("./OutSettingTmp.json");
  ...
  SaveAttrJSON(JSONtmp,"path_Lib",path_Lib);
  SaveAttrJSON(JSONtmp,"path_Raw",path_Raw);
  SaveAttrJSON(JSONtmp,"path_Hk" ,path_Hk );
  SaveAttrJSON(JSONtmp,"path_Run",path_Run);
  SaveAttrJSON(JSONtmp,"path_Rot",path_Rot);
  SaveAttrJSON(JSONtmp,"path_OpR",path_OpR);
  SaveAttrJSON(JSONtmp,"path_Mac",path_Mac);
  SaveAttrJSON(JSONtmp,"name_Raw",name_Raw);
  SaveAttrJSON(JSONtmp,"name_Hk" ,name_Hk );
  SaveAttrJSON(JSONtmp,"runOPT"  ,runOPT  );
  EndWriteJSON(JSONtmp);
  ShowJSONFile(JSONtmp);
  cout<<"Save setting to be "<<JSONtmp<<endl;
  ...
  string vertmp;
  for (int i=0; DataDetVer[i]!="-" ;i++)
    if(int( runOPT.find(DataDetVer[i])) != -1) vertmp = DataDetVer[i];
  cout<<"Find vertmp is: "<<vertmp<<endl;
  ...
  bool SkipBlocks = true;
    if(BlockEnv){
      ...
      cout<<vertmp.data()<<" "<<TString(path_Mac)<<" "<<TString(path_Lib)<<endl;
      // throw;
      RenewMacrosV2(vertmp.data(),TString(path_Mac),TString(path_Lib));
      /* const path of Macro compiling */
  ...
  if(!SkipBlocks){
    if(BlockODetImf){
      ...
      ODectImfAna();
      TillRunODInf();
      EffTillRunODInf("/home/yusiang/TestV2/EffV0305N0_New.txt");
  ...
  if(!SkipBlocks){
    ...
    if(BlockEventGaps){
      /* P.12 *///-New-
      /*Convert Raw_Mu.root to be Gap*.root by tcnt cut*/
      gROOT->LoadMacro(Form("%sEventAnaV2.C+",path_Mac));
      //int Num_RawEVE = EventAna(33,34);
      //void EventAna(const int indexi=28, const int indexf=29, const int unixtimeini = unixtimei, const int unixtimefin = unixtimef, const char * runName="-NA-" ) {//140
      EventAnaV2(33,34);
      // EventGapAna();
